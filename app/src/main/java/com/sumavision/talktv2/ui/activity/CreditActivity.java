...
public class CreditActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mBackView.setBackgroundResource(R.mipmap.integral_mall_btn);

		// 添加后退按钮监听事件
		mBackView.setOnClickListener(new NoDoubleClickListener() {
  ...
  //自定义导航栏，包含 后退按钮，页面标题，分享按钮（默认隐藏）
	...
	protected void onResume() {
		...
		if (ifRefresh) {
			...
			finish();
			mWebView.reload();
			//如果是活动中心过来的登陆成功的状态,我需要发送消息通知刷新界面
			BusProvider.getInstance().post("refreshIntg","refreshIntg");
